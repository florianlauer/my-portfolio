...
- **Usage** : setup `devenv` actif via `devenv hook zsh`. Les commandes `npm`, `npx`, `node` sont lancées **directement** dans le terminal du repo (environnement déjà chargé).
  - `devenv shell`/`devenv run` reste un fallback si le shell n’est pas auto-activé.
- **Ne pas supposer** une installation globale système ; l’environnement outillé provient de `devenv`.
